...
public class SpacedRepetitionBot extends CommandLongPollingTelegramBot {
     ...
     * @warning Команды, которые имеют корректный синтаксис, но не зарегистрированы на этом боте, не будут форвардиться в этот метод, если есть дефолтное действие.
     */
    @Override
     ...
     * переопределите его в своей реализации, если хотите, чтобы ваш бот выполнял другие действия, например, отправлял сообщение об ошибке.
     ...
     * Например, если вы хотите запретить выполнение команд, приходящих из чата группы: return !message.getChat().isGroupChat();
     * @param message Полученное сообщение
     * @return true, если сообщение должно быть проигнорировано ботом и обработано как не-команда, false в противном случае
